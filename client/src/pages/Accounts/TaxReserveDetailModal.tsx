...
import { Loader2, Receipt, AlertTriangle, CheckCircle2, Pencil } from 'lucide-react'
import { UiModal, UiModalHeader, UiModalBody } from '../../components/ui/UiModal'
import { fetchTaxBreakdown, type TaxReserveBreakdown, type TaxReserveBreakdownItem, type AccountWithBalance } from '../../services/accountService'
...
export function TaxReserveDetailModal({ account, isOpen, onClose, onEditAccount }: Props) {
  const { t, language } = useI18n()
  const [data, setData] = useState<TaxReserveBreakdown | null>(null)
  ...
  // Una porción por proyecto (mismo proyecto = mismo color); slices < 2% en
  // bucket "Otros" para que el chart sea legible. La lista lateral muestra todos.
  const { sliceData, listItems } = useMemo(() => {
    ...
    const list: SliceItem[] = data.items.map((it, idx) => {
      ...
      if (!(key in projectColors)) {
        projectColors[key] = PALETTE[pIdx % PALETTE.length]
        pIdx++
      }
  ...
  return (
    <UiModal isOpen={isOpen} onClose={onClose} width="3xl">
      <UiModalHeader>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: '15px', fontWeight: 700 }}>{account.name}</div>
              <div style={{ fontSize: '11px', color: 'var(--text-muted, #6b7280)', marginTop: '1px' }}>
                {t('accounts.taxReserve.subtitle') || 'Reserva de impuestos · desglose por movimiento'}
              </div>
            </div>
          </div>
          <button
      ...
      <UiModalBody>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '18px' }}>
          ...
          <Stat
            ...
            value={fmt(deltaAbs)}
            hint={
              isExcess ? 'Saldo > ingresos contabilizados'
              : isShort ? 'Ingresos > saldo (revisa movimientos)'
              : 'Cuadra perfectamente'
            }
            icon={
              isExcess ? <CheckCircle2 size={13} style={{ color: '#10b981' }} />
              : isShort ? <AlertTriangle size={13} style={{ color: '#f59e0b' }} />
            ...
            <div style={{ position: 'relative', height: '280px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={sliceData}
                    dataKey="irpf_contribution"
                    nameKey="transaction_id"
                    cx="50%" cy="50%"
            ...
            <div style={{ maxHeight: '320px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {listItems.map(it => (
                <Row
                  ...
                  onHover={() => {
                    const idx = sliceData.findIndex(s => s.transaction_id === it.transaction_id)
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{
          ...
          {dateStr}
          {item.project?.display_id && (<> · {item.project.display_id}</>)}
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        color: '#6366f1', marginBottom: '14px',
      }}>
        <Receipt size={22} />
      </div>
      ...
      <div style={{ fontSize: '12px', marginTop: '6px', maxWidth: '420px', lineHeight: 1.5 }}>
        Cuando hagas una transferencia o registres un ingreso a esta cuenta, aparecerá aquí su porción del pie.
      </div>
    </div>
  )
}
